...
class HRDAHead(BaseDecodeHead):
    ...
    def forward_target(self,
        ...
        seg_logits = self.forward(inputs)
        # pdb.set_trace()
        losses = self.losses_ssda(inputs[0][0],seg_logits, gt_semantic_seg, seg_weight,conf_mask,uncertainty,mres_feats)
        self.reset_crop()
        return losses
    ...
    def forward_pseudo(self, inputs, img_metas, test_cfg):
        assert len(inputs) == 2
        hr_inp = inputs[1]
        hr_scale = self.scales[1]
        lr_inp = inputs[0]
        ...
        # print_log(f'lr_inp {[f.shape for f in lr_inp]}', 'mmseg')
        lr_seg = self.head(lr_inp)###lr_seg.shape torch.Size([2, 19, 64, 64])

        # print_log(f'lr_seg {lr_seg.shape}', 'mmseg')

        hr_seg = self.decode_hr(hr_inp, batch_size)###hr_seg.shape torch.Size([2, 19, 64, 64])

        # pdb.set_trace()

        att = self.get_scale_attention(lr_sc_att_inp)###([2, 19, 64, 64])
        if has_crop:
            mask = lr_seg.new_zeros([lr_seg.shape[0], 1, *lr_seg.shape[2:]])
            sc_os = self.os / lr_scale
            slc = self.hr_crop_slice(sc_os)
            ...
            att = att * mask
        # print_log(f'att {att.shape}', 'mmseg')
        lr_seg = (1 - att) * lr_seg##([2, 19, 64, 64])
        # print_log(f'scaled lr_seg {lr_seg.shape}', 'mmseg')
        up_lr_seg = self.resize(lr_seg, hr_scale / lr_scale)###([2, 19, 128, 128])
        if torch.is_tensor(att):
            att = self.resize(att, hr_scale / lr_scale)###torch.Size([2, 19, 128, 128])
        ...
        if self.debug:
            ...
            if torch.is_tensor(att):
                self.debug_output['Attention'] = torch.sum(
                    att * torch.softmax(fused_seg, dim=1), dim=1,
                    keepdim=True).detach().cpu().numpy()
        return fused_seg, up_lr_seg, hr_seg_inserted
    ...
    def losses_ssda(self, target_img,seg_logit, seg_label, seg_weight=None,conf_mask=None,uncertainty = None,mres_feats = None):
        """Compute losses."""
        fused_seg, lr_seg, hr_seg = seg_logit
        if self.hr_loss_weight == 0 and self.lr_loss_weight == 0:
            return loss
        seg_weight = seg_weight*conf_mask
        loss = super(HRDAHead, self).losses(fused_seg, seg_label, seg_weight)
        gussxy=3
        gussrgb=0.005
        gussr=5
        fused_seg = self.resize(fused_seg, 0.25)
        lambda_0 = 1
        lambda_1 =  1
        lambda_2 =  0.1
        loss['loss_seg'] += lambda_1*uncertainty
        ...
        if self.hr_loss_weight > 0 and self.enable_hr_crop:#####0.1,true
            ...
            loss.update(
                add_prefix(
                    super(HRDAHead, self).losses(hr_seg, seg_label,
                                                 seg_weight), 'hr'))
        loss['loss_seg'] *= (1 - self.lr_loss_weight - self.hr_loss_weight)*0.01
        if self.lr_loss_weight > 0:
            loss['lr.loss_seg'] *= self.lr_loss_weight*0.01
        if self.hr_loss_weight > 0:
            loss['hr.loss_seg'] *= self.hr_loss_weight*0.01
