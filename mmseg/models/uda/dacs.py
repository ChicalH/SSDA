...
import pdb
import pandas as pd
from mmseg.models.losses.func import ModelLossSemsegGatedCRF
...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        self.pseudo_threshold = cfg['pseudo_threshold']#0.968
        self.psweight_ignore_top = cfg['pseudo_weight_ignore_top']##0
        self.psweight_ignore_bottom = cfg['pseudo_weight_ignore_bottom']##0
        ...
        self.loss_gatedcrf_main = ModelLossSemsegGatedCRF()
    ...
    def calc_SSDA(self, target_results, target_img, pseudo_label, conf_mask=None):

        T_result=[torch.softmax(x.detach(), dim=1) for x in target_results]
        t = sum(T_result)/len(T_result)
        def entropy_map(v):
            return -torch.sum(torch.mul(v, torch.log2(v + 1e-30)),dim=1)
        item1 = entropy_map(t)
        item2 = [entropy_map(x) for x in T_result]
        item2 = sum(item2)/len(T_result)
        seg_debug['Target'] = self.get_ema_model().decode_head.debug_output
        pseudo_prob, pseudo_label = torch.max(T_result[0], dim=1)
        ps_large_p = pseudo_prob.ge(self.pseudo_threshold).long() == 1##[2, 1024, 1024])
        ps_size = np.size(np.array(pseudo_label.cpu()))
        pseudo_weight = torch.sum(ps_large_p).item() / ps_size
        shape__ = pseudo_prob.shape
        ...
        conf_mask = torch.zeros(shape__).to(T_result[0].device)
        # print(shape__)
        beta=0.1
        for cla in range(self.num_classes):
            mask_class = (pseudo_label == cla)
            label_ratio = (1-np.log(weighted_softmax[cla]))*beta
            if label_ratio>1.0:
                label_ratio = label_ratio*0+1
            if torch.sum(mask_class)>10:
                pro_class = p_easy[mask_class]
                topk_num = np.ceil(torch.sum(mask_class).cpu().numpy()*label_ratio.numpy())
                pro_class_top,_= pro_class.topk(int(topk_num))
                thr = pro_class_top.min()
                conf_mask += torch.ge(torch.mul(p_easy,mask_class), thr)
        del weighted_softmax, A,p_easy


        gussxy=3
        gussrgb=0.005
        gussr=5
        loss_crf = self.loss_gatedcrf_main(
            F.softmax(target_results[0],dim=1),
            kernels_desc = [{"weight":1,"xy":gussxy,"rgb":gussrgb}],
            kernels_radius = gussr,
            sample=target_img,####
            height_input=target_results.shape[-2],
            width_input = target_results.shape[-1],
            mask_src = conf_mask.unsqueeze(1).detach(),
            mask_dst=(-(conf_mask-1)).unsqueeze(1)
            )
        loss_crf = loss_crf['loss'].mean()
        loss_crf= torch.mean(loss_crf)

        loss_uncertainty=torch.sum(B) / (n * h * w )

        lambda_0 = 1
        lambda_1 =  1
        lambda_2 =  0.1
        loss_target = lambda_0*loss_seed+lambda_1*loss_uncertainty+lambda_2* loss_crf

        target_loss, target_log = self._parse_losses(
            {'loss_target': loss_target})
        target_log.pop('loss', None)


        
        return target_loss, target_log
    ...
    def forward_train(self,
        ...
        seed = True
        ori_pseudo_weight = True
        ema_logits = self.get_ema_model().generate_pseudo_label(target_img, target_img_metas)
        seg_debug['Target'] = self.get_ema_model().decode_head.debug_output
        ema_softmax = torch.softmax(ema_logits.detach(), dim=1)
        del ema_logits
        pseudo_prob, pseudo_label = torch.max(ema_softmax, dim=1)
        n,h,w = pseudo_prob.size()
        T_result = [ema_softmax]
        for i in range(1):
            ema_logits = self.get_ema_model().generate_pseudo_label(target_img, target_img_metas)###target_img,1024*1024
            T_result.append(torch.softmax(ema_logits.detach(), dim=1))
            del ema_logits 
        t = sum(T_result)/len(T_result)
        def entropy_map(v):
            return -torch.sum(torch.mul(v, torch.log2(v + 1e-30)),dim=1)
        ...
        weighted_softmax = torch.from_numpy(weighted_softmax)
        weighted_softmax = weighted_softmax / torch.sum(weighted_softmax)
        conf_mask = torch.zeros(pseudo_prob.shape).to(T_result[0].device)
        # print(shape__)
        beta=0.1
        for cla in range(self.num_classes):
            mask_class = (pseudo_label == cla)
            ...
            if torch.sum(mask_class)>10:
                ...
                thr = pro_class_top.min()
                conf_mask += torch.ge(torch.mul(p_easy,mask_class), thr)
        del weighted_softmax, A,p_easy,B,item1,item2
        ...
        if self.psweight_ignore_bottom > 0:##0
            assert valid_pseudo_mask is None
            pseudo_weight[:, -self.psweight_ignore_bottom:, :] = 0
        if valid_pseudo_mask is not None:###True
            pseudo_weight *= valid_pseudo_mask.squeeze(1)
        
        pseudo_weight_target = pseudo_weight
        ...
        for i in range(batch_size):
            ...
            _, pseudo_weight[i] = strong_transform(################ domain mask
                strong_parameters,
        ...
        if True:
            target_loss = self.get_model().forward_target(target_img,target_img_metas,
            pseudo_label.unsqueeze(1),pseudo_weight_target,conf_mask = conf_mask,uncertainty = uncertainty)
            target_loss = add_prefix(target_loss, 'target')
            target_loss, target_log_vars = self._parse_losses(target_loss)
            log_vars.update(target_log_vars)
            target_loss.backward(retain_graph=self.enable_fdist)
