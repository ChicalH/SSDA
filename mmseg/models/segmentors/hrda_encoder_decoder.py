...
class HRDAEncoderDecoder(EncoderDecoder):
    ...
    def generate_pseudo_label_SSDA(self, img, img_metas):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        mres_feats = []
        self.decode_head.debug_output = {}
        for i, s in enumerate(self.scales):
            ...
            if self.decode_head.debug:
                self.decode_head.debug_output[f'Img {i} Scale {s}'] = \
                    scaled_img.detach()
        out = self.decode_head.forward_pseudo(mres_feats,img_metas, self.test_cfg)
        out = [resize(
            input=x,
            size=img.shape[2:],
            mode='bilinear',
            align_corners=self.align_corners) for x in out]
        return out
    ...
    def generate_pseudo_label(self, img, img_metas):
        ...
        for i, s in enumerate(self.scales):
            ...
            if self.decode_head.debug:
                self.decode_head.debug_output[f'Img {i} Scale {s}'] = \
                    scaled_img.detach()
        out = self.decode_head.forward_test(mres_feats,img_metas, self.test_cfg)
        out = resize(
            input=out,
            size=img.shape[2:],
            mode='bilinear',
    ...
    def forward_target(self,
        ...
        loss_decode = self._decode_head_forward_target(mres_feats, img_metas,
                                                      gt_semantic_seg,
                                                      seg_weight,conf_mask,uncertainty,mres_feats[0][0])
